...
class WonderPlugin_Gallery_View {
	...
	function print_register() {
		...
		<h2><?php _e( 'License', 'wonderplugin_gallery' ); ?></h2>
		<?php
				
		if (isset($_POST['save-gallery-license']))
		{		
		...
		else if (isset($_POST['deregister-gallery-license']))
		{	
		...
		else
		{
			...
			<?php } else { ?>
				...
				<tr>
					...
					<td><?php echo $key; ?> &nbsp;&nbsp;&nbsp;&nbsp;<input name="wonderplugin-gallery-key" type="hidden" id="wonderplugin-gallery-key" value="<?php echo $key; ?>" class="regular-text" /><input type="submit" name="deregister-gallery-license" id="deregister-gallery-license" class="button button-primary" value="Deregister the License Key"  /></label></td>
				</tr>
				</table>
				</form>
				...
				<tr>
					<th></th>
					<td>
					<p><strong>Your license key is valid.</strong></p><?php if ( !empty($info->registered_domain) ) echo "<p>The key is registered to the domain <strong>" . $info->registered_domain . "</strong></p>"; ?></td>
				</tr>
	...
	function print_add_new() {
		...
		if ( !empty($_POST['wonderplugin-gallery-save-item-post-value']) && !empty($_POST['wonderplugin-gallery-save-item-post']) )
		{
	...
	function save_item_post($item_post) {
		...
		if (isset($ret['success']) && $ret['success'] && isset($ret['id']) && $ret['id'] >= 0) 
		{
			...
			echo "<div class='updated'><p>To embed the gallery into your page or post, use shortcode <b>[wonderplugin_gallery id=\"" . $ret['id'] . "\"]</b></p></div>";
			echo "<div class='updated'><p>To embed the gallery into your template, use php code <b>&lt;?php echo do_shortcode('[wonderplugin_gallery id=\"" . $ret['id'] . "\"]'); ?&gt;</b></p></div>"; 
		}
		else
		{
