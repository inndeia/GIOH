...
class WonderPlugin_Gallery_Update {
	...
	function plugin_update_notice() {
		...
		if ( !empty($info->notice) )
			echo '<div class="error"><p>'. $info->notice . '</p></div>';
	}
	...
	function get_update_data($action, $key = '', $force = false) {
		...
		$info = $this->controller->get_plugin_info();
		if (!$force && ($action == 'basic_check') && !empty($info) && isset($info->last_checked) && is_int($info->last_checked) && ((time() - $info->last_checked) < 12 * 60 * 60))
			return $info;
	...
	function show_plugin_update_message( $plugin_data, $r ) {
		...
		{
			echo '<br>Your free upgrade period has expired. To get upgrades, please <a href="https://www.wonderplugin.com/renew/" target="_blank">renew your license</a>.';
		}
		else
		{
			if (WONDERPLUGIN_GALLERY_VERSION_TYPE == 'F')
			{
				echo '<br>To get upgrades, please <a href="https://www.wonderplugin.com/order/" target="_blank">upgrade to a commercial license</a>.';
			}
			else
			{
				echo '<br>To get upgrades, please <a href="'. admin_url('admin.php?page=wonderplugin_gallery_register') . '">enter your license key</a> or <a href="https://www.wonderplugin.com/register-faq/#manualupdate" target="_blank">update the plugin manually</a>.';
			}
		}
	}
}